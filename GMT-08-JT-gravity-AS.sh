#!/bin/sh
# Purpose: Gravity model map
# Mercator projection (here: Indian Ocean).
# GMT modules: gmtset, img2grd, grd2cpt, grdimage, pscoast, psbasemap, psscale, logo, pstext, psconvert

# GMT set up
gmt set FORMAT_GEO_MAP=dddF \
    MAP_TITLE_OFFSET=1c \
    MAP_FRAME_PEN=dimgray \
    MAP_FRAME_WIDTH=0.1c \
    MAP_TICK_PEN_PRIMARY=thinner,dimgray \
    MAP_GRID_PEN_PRIMARY=thinnest,gray30 \
    MAP_GRID_CROSS_SIZE_PRIMARY=0.5c \
    MAP_ANNOT_OFFSET=0.1c \
    FONT_TITLE=12p,Palatino-Roman,black \
    FONT_ANNOT_PRIMARY=7p,Helvetica,dimgray \
    FONT_LABEL=7p,Helvetica,dimgray \
    MAP_LABEL_OFFSET=0.1c
    
# Extract subset of img file in Mercator or Geographic format
img2grd grav_27.1.img -R47/77/0/31 -Ggrav.grd -T1 -I1 -E -S0.1 -V

grdcut grav.grd -R47/77/0/31 -Gas_grav.nc

# Generate a color palette table from grid
gdalinfo as_grav.nc -stats
#Minimum=-155.097, Maximum=366.939
#gmt grd2cpt grav.grd -Crainbow > grav.cpt
#gmt makecpt -Chaxby.cpt -V -T-155/366/10 > colors.cpt
gmt makecpt -Chaxby.cpt -V -T-106/85/1 > colors.cpt

# Generate a file
ps=Grav_AS.ps
# Generate gravity image with shading
gmt grdimage grav.grd -I+a45+nt1 -R47/77/0/31 -JT62/15/6i -Ccolors.cpt -P -K > $ps

# Add basemap: grid, title, coastline
gmt pscoast -R -J -P \
    -V -W0.25p \
    -Df -B+t"Marine free-air gravity anomaly: Arabian Sea region" \
    --MAP_TITLE_OFFSET=1.1c \
    --FONT_TITLE=14p,Palatino-Roman,black \
    --FONT_ANNOT_PRIMARY=8p,Helvetica,black \
    --FONT_LABEL=8p,Helvetica,black \
    -Bpx104f2.5a5 -Bpyg10f2.5a5 -Bsxg2.5 -Bsyg2.5 \
    -O -K >> $ps

gmt grdcontour grav.grd -R -J -C30 -W0.1p -O -K >> $ps

# Add scale, directional rose
gmt psbasemap -R -J \
    --FONT=9p,Helvetica,black \
    --MAP_TITLE_OFFSET=0.3c \
    -Tdx12.6c/14.0c+w0.3i+f2+l+o0.15i \
    -Lx12c/-2.7c+c50+w1000k+l"Transverse Mercator projection. Scale: km"+f \
    -UBL/-5p/-75p -O -K >> $ps
    
# Add legend
gmt psscale -Dg47/-2.8+w15.0c/0.4c+h+o0.3/0i+ml -R47/77/0/31 -J -Ccolors.cpt \
    --FONT_LABEL=8p,Helvetica,black \
    --FONT_ANNOT_PRIMARY=8p,Helvetica,black \
    --MAP_ANNOT_OFFSET=0.1c \
    -Ba10f2+l"Color scale: Haxby: Bill Haxby's color scheme for geoid & gravity [C=RGB]" \
    -I0.2 -By+lmGal -O -K >> $ps
    
# Texts
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times−Bold,blue+jLB >> $ps << EOF
63.2 20.8 Oman
63.2 20.2 Abyssal
63.2 19.5 Plain
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,white+jLB+a-290 >> $ps << EOF
60.6 16.8 Owen
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,white+jLB+a-300 >> $ps << EOF
61.3 18.8 Fracture
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,white+jLB+a-310 >> $ps << EOF
62.7 21.2 Zone
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,black+jLB -Gwhite@40 >> $ps << EOF
59.0 23.15 Makran Trench
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times−Bold,black+jLB+a-335 -Gwhite@40 >> $ps << EOF
48.2 12.5 Gulf of Aden
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times−Bold,black+jLB+a-53 -Gwhite@40 >> $ps << EOF
49.5 29.0 P e r s i a n  G u l f
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times−Bold,black+jLB -Gwhite@40 >> $ps << EOF
56.5 21.5 OMAN
62.5 27.0 P A K I S T A N
54.0 30.2 I R A N
74.0 22.0 I N D I A
49.0 22.0 SAUDI
49.0 21.3 ARABIA
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,Times-Roman,blue+jLB >> $ps << EOF
61.0 15.5 ARABIAN
62.2 14.0 SEA
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,Times-Roman,black+jLB+a-300 -Gwhite@50 >> $ps << EOF
48.0 6.0 S O M A L I
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times-Roman,red+jLB+a-270 -Gwhite@60 >> $ps << EOF
74.0 2.0 M  a  l  d  i  v  e  s
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times−Bold,black+jLB -Gwhite@35 >> $ps << EOF
53.0 11.8 Socotra
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Times-Roman,black+jLB+a-333 -Gwhite@50 >> $ps << EOF
48.5 15.1 Y E M E N
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,black+jLB+a-40 -Gwhite@35 >> $ps << EOF
58.8 6.7 C a r l s b e r g  R i d g e
EOF
    
# Add GMT logo
gmt logo -Dx6.2/-3.4+o0.1i/0.1i+w2c -O -K >> $ps

# Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X0.5c -Y6.7c -N -O \
    -F+f12p,Palatino-Roman,black+jLB >> $ps << EOF
0.8 15.5 Global gravity grid from CryoSat-2 and Jason-1, 1 min resolution
0.5 14.8 Transverse Mercator prj. Central meridian: 62\232E Standard parallel: 15\232N
EOF

# Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Grav_AS.ps -A1.5c -E720 -Tj -P -Z
