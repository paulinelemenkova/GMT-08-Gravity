...
# Mercator projection (here: South China Sea).
...
img2grd grav_27.1.img -R99/122/2/23 -GgravSCS.grd -T1 -I1 -E -S0.1 -V
# Step-4. Generate a color palette table from grid
gmt grd2cpt gravSCS.grd -Crainbow > gravSCS.cpt
# Step-5. Generate gravity image with shading
gmt grdimage gravSCS.grd -I+a45+nt1 -R99/122/2/23 -JY111/12/6.5i -CgravSCS.cpt -P -K > $ps
# Step-6. Add basemap: grid, title, costline
gmt pscoast -R -J -P \
	-V -W0.25p \
    -Df -B+t"Marine free-air gravity anomaly for the South China Sea" \
	-Bxg3f2a4 -Byg3f2a4 \
    -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_TITLE_OFFSET=.3c \
    -Tdg101.5/4.5+w1.0c+f2+l \
    -Lx5.0i/-0.5i+c50+w500k+l"Cylindrical Equal-Area Gall-Peters projection. Scale, km"+f \
    -UBL/-15p/-40p -O -K >> $ps
# Step-8. Add color legen3
gmt psscale -R -J -CgravSCS.cpt \
    -Dg96/2+w15.0c/0.5c+v+o0.7/0i+ml  \
    --FONT_LABEL=7p,Helvetica,dimgray \
...
gmt pstext -R0/10/0/15 -JX10/10 -X0.0c -Y6.0c -N -O \
...
gmt psconvert Grav_SCS.ps -A0.2c -E720 -Tj -P -Z
