...
img2grd curv_27.1.img -R154/220/40/65 -GcurvAT.grd -T1 -I1 -E -S0.1 -V
# Step-4. Generate a color palette table from grid
gmt grd2cpt curvAT.grd -CGMT_haxby > curvAT.cpt
# Step-5. Generate gravity image with shading
gmt grdimage curvAT.grd -I+a45+nt1 -R154/220/40/65 -JY188/55/6.5i -CcurvAT.cpt -P -K > $ps
# Step-6. Add basemap: grid, title, costline
gmt pscoast -R -J -P \
	-V -W0.25p \
    -Df -B+t"Marine free-air vertical gravity anomaly for the Aleutian Trench area" \
	-Bxg3f4a8 -Byg3f2a4 \
    -O -K >> $ps
...
gmt psconvert Curv_AT.ps -A0.2c -E720 -Tj -P -Z
