...
gmt psbasemap -R -J \
    --FONT=10p,Helvetica,dimgray \
    --MAP_TITLE_OFFSET=0.3c \
    -Lx13c/-0.5i+c50+w800k+l"Oblique Mercator projection. Scale (km)"+f \
    -O -K >> $ps
# texts
...
EOF
gmt pstext -R -J -N -O -K \
-F+f10p,Helvetica−Bold,black+jLB+a-300 -Gwhite@30 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,darkblue+jLB -Gwhite@30 >> $ps << EOF
153.5 -38.5 T A S M A N  S E A
177.5 -52 P A C I F I C
177.5 -53 O C E A N
EOF
gmt pstext -R -J -N -O -K \
-F+f10p,Helvetica−Bold,red+jLB+a-290 -Gwhite@30>> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f10p,Helvetica−Bold,red+jLB+a-300 -Gwhite@30>> $ps << EOF
...
1.5 16.0 Global gravity grid from CryoSat-2 and Jason-1, 1 min resolution
1.5 15.3 Oblique Mercator projection (-JOc) parameters: center 165\232/-29.5\232; pole -100\232/85\232.
EOF
