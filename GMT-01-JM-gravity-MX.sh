...
gmt img2grd grav_27.1.img -R240/275/14/33 -Ggrav_MX.grd -T1 -I1 -E -S0.1 -V
gdalinfo grav_MX.grd -stats
# Minimum=-223.503, Maximum=418.827, Mean=-2.211, StdDev=36.694

# Generate a color palette table from grid
# gmt makecpt --help
# gmt makecpt -Chaxby -T-342/460 > colors.cpt
gmt makecpt -Chsv.cpt -T-150/150 > colors.cpt
#-Ic Reverse sense of color table spectrum

# Generate a file
ps=Grav_MX.ps

gmt grdimage grav_MX.grd -Ccolors.cpt -R240/275/14/33 -JM6.5i -P -I+a15+ne0.75 -Xc -K > $ps

# Add isolines
gmt grdcontour grav_MX.grd -R -J -C100 -A200 -Wthinner -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Free-air gravity anomaly for Mexico" -O -K >> $ps
...
gmt psscale -Dg240/11.7+w16.5c/0.4c+h+o0.0/0i+ml+e -R -J -Ccolors.cpt \
    ...
    -Bg10f2a20+l"Color scale 'hsv' of h5utils package by MIT's S.G. Johnson [continuous, RGB, 63 segments -T-150/150]" \
...
gmt psconvert Grav_MX.ps -A0.5c -E720 -Tj -Z
