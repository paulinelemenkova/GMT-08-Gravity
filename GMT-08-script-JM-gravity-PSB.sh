...
# Mercator projection (here: Philippine Sea Basin).
...
img2grd grav_27.1.img -R120/152/4/35 -Ggrav.grd -T1 -I1 -E -S0.1 -V
# Step-4. Generate a color palette table from grid
gmt grd2cpt grav.grd -Cno_green > grav.cpt
...
gmt pscoast -R -J -P \
	-V -W0.25p \
    -Df -B+t"Marine free-air gravity anomaly for the Philippine Sea Basin" \
	-Bxa4g3f2 -Bya4g3f2 \
    -O -K >> $ps
...
gmt psscale -R -J -Cgrav.cpt \
    -Dg114.5/4+w15.8c/0.5c+v+o0.5/0i+ml  \
    --FONT_LABEL=7p,Helvetica,dimgray \
    --FONT_ANNOT_PRIMARY=7p,Helvetica,dimgray \
    --MAP_ANNOT_OFFSET=0.1c \
    --MAP_LABEL_OFFSET=0.1c \
...
3.0 20.3 Global gravity grid from CryoSat-2 and Jason-1, 1 min resolution
EOF
# Step-11. Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Grav_PSB.ps -A0.2c -E720 -Tj -P -Z
