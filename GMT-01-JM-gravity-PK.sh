...
gmt img2grd grav_27.1.img -R60.0/80.0/23.5/37.2 -GgravPK.grd -T1 -I1 -E -S0.1 -V
gdalinfo gravPK.grd -stats
# Minimum=-353.707, Maximum=611.238

# Generate a color palette table from grid
# gmt makecpt -Cturbo -T-288/487/1 > colors.cpt
gmt makecpt -Cturbo -T-354/612/10 > colors.cpt
# -Ic

# Generate a file
ps=Grav_PK.ps
gmt grdimage gravPK.grd -Cturbo -R60.0/80.0/23.5/37.2 -JM6.5i -P -I+a15+ne0.75 -Xc -K > $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Free-air gravity anomaly for Pakistan" \
...
gmt psscale -Dg60.0/22.0+w16.0c/0.15i+h+o0.3/0i+ml -R -J -Ccolors.cpt \
    ...
    -Bg50f10a50+l"Color scale 'turbo': Google's Improved Rainbow Colormap for Visualization [C=RGB] -354/612)" \
